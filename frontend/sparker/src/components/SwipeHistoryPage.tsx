		...
		<div className="global-container">
			<div className="swipes-container">
				...
				<ul className="swipe-list">
					{swipes.length <= 0 && (
						<div>
							<p className="empty-list">You have no swipes.</p>
							<button
					...
					{swipes.map((swipe) => (
						<li
							key={swipe.id}
							onClick={() =>
								handleSwipeClick(swipe.id, swipe.swipedUserId)
							}
							className={`swipe-item ${
								swipe.liked ? "swipe-liked" : "swipe-passed"
							}`}
						>
							...
							<div className="swipeuser-title">
								{swipe.swipedName} - Swiped on{" "}
								{new Date(swipe.swipedAt).toLocaleDateString()}
							</div>
							<div>
								{swipe.swipedAge}, {swipe.swipedGender}
							</div>
							<div>
								{swipe.liked ? "You liked " : "You passed on "}{" "}
								{swipe.swipedName}
							</div>
							<div>
